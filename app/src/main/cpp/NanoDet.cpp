...
std::vector<BoxInfo> NanoDet::detect(JNIEnv *env, jobject image, float score_threshold, float nms_threshold) {
    ...
    ncnn::Mat output(out.h, out.w);
    for (int h = 0; h < out.h; ++h) {
        //auto channel = out.channel(h); // Assuming channel(h) returns a pointer or a reference to the channel
        for (int w = 0; w < out.w; ++w) {
            output.row(w)[h] = out.row(h)[w];
            //output[h][w] = channel[w]; // Access the channel data and assign it to output
        }
    }
    ...
    this->decode_infer(output, center_priors, score_threshold, results, width_ratio, height_ratio);
